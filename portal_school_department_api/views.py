...
class AdminDeactivateSchoolDepartmentAPIView(APIView):
    permission_classes = [IsAuthenticated]
    serializer_class = AdminDeactivateDepartmentSerializer
    ...
    def post(self, request):
        try:
            ...
            if  school.status == 2:
                return Response({
                    'status': False,
                    'message': f'{school_name} is deactivated.'
                }, status=status.HTTP_400_BAD_REQUEST)
            ...
            if  school_department.status == 2:
                return Response({
                    'status': False,
                    'message': f'{department_name} is already deactivated.'
                }, status=status.HTTP_400_BAD_REQUEST)
            

            school_department.status = 2
            school_department.save()

            print("school department deactivated successfully")
