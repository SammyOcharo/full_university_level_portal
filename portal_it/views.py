    ...
    def post(self, request):
        try:
            ...
            if  it_user.status == 2:
                return Response({
                    'status': False,
                    'message': f'it admin{it_user.full_name} is already suspended'
                }, status=status.HTTP_400_BAD_REQUEST)
            
            if  it_user.status == 3:
                return Response({
                    'status': False,
                    'message': f'it admin{it_user.full_name} is deactivated, you cannot suspend a deactivated account!'
                }, status=status.HTTP_400_BAD_REQUEST)
            

            it_user.status = 2
            it_user.save()
            print("it admin suspended!")

            return Response({
                'status': True,
                'message': f' it admin with first name {it_user.full_name} and email {it_user.email} is Suspended!'
            }, status=status.HTTP_200_OK)
            ...
            return Response({
                'status': False,
                'message': 'Could not suspended it admin.'
            }, status=status.HTTP_400_BAD_REQUEST)
    ...
    def post(self, request):
        try:
            ...
            if  it_user.status == 3:
                return Response({
                    'status': False,
                    'message': f'it admin{it_user.full_name} is already deactivated'
                }, status=status.HTTP_400_BAD_REQUEST)
            

            it_user.status = 3
            it_user.save()
            print("it admin deactivated!")

            return Response({
                'status': True,
                'message': f' it admin with first name {it_user.full_name} and email {it_user.email} is Deactivated!'
            }, status=status.HTTP_200_OK)
            ...
            return Response({
                'status': False,
                'message': 'Could not deactivated it admin.'
            }, status=status.HTTP_400_BAD_REQUEST)
    ...
    def post(self, request):
        try:
            ...
            if  it_user.status == 1:
                return Response({
                    'status': False,
                    'message': f'it admin{it_user.full_name} is already activate'
                }, status=status.HTTP_400_BAD_REQUEST)
            

            it_user.status = 1
            it_user.save()
            print("it admin reactivated!")

            return Response({
                'status': True,
                'message': f' it admin with first name {it_user.full_name} and email {it_user.email} is Reactivated!'
            }, status=status.HTTP_200_OK)
            ...
            return Response({
                'status': False,
                'message': 'Could not reactivated it admin.'
            }, status=status.HTTP_400_BAD_REQUEST)
